...
| 純粋関数の強制 | Haskell: IO モナドで分離 / Elm: Cmd/Sub | `trill` によるオプトイン強制。`trill meow` の本体では他の `trill` 関数と副作用のない組み込みのみ呼び出せる（`nya`/`hiss`/`gag`・インポートパッケージ・非 `trill` 関数の呼び出しはコンパイルエラー） |
| 不変変数の宣言 | 全変数がデフォルト不変 | 全変数がデフォルト不変（同一スコープでの再宣言はエラー。内側スコープでのシャドーイングは可） |

> **`trill` の既知の限界（ステップ1）**: 非純粋関数を「呼び出さず、第一級の値として渡す（裸の識別子）」ケースは検出しない。ここは将来拡張とする。
